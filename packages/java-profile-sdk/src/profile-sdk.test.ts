...
describe("JavaProfileSdk", () => {
  describe("constructor", () => {
    ...
    it("uses custom provider instance if specified", () => {
      ...
      const sdk = new JavaProfileSdk({ provider: customProvider });
      expect(sdk.provider).toBe(customProvider);
    });
    it("uses specific chainId if chainId is provided", () => {
      const sdk = new JavaProfileSdk({ chainId: TESTNET_CHAIN_ID });
    ...
    describe("getProfile", () => {
      ...
      it("sets cookies", async () => {
        ...
        expect(Cookies.set).toBeCalledWith(
          ...
          {
            username: "Cheems",
            avatar: `https://javaswap.io/images/nfts/${sleepyNft.images.sm}`,
          },
